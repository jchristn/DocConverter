...
rem  IsPackable=true produce packages (DocConverter and the DocConverter.Cli
rem  tool); the test projects are skipped.
rem
...
if not exist "%OUTPUT%\DocConverter.*.nupkg" (
    echo No DocConverter .nupkg files were produced in %OUTPUT%.
    goto :error
)
