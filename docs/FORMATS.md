...
| XLSX | DocumentFormat.OpenXml | One section per visible sheet holding a table over the used range, detected header row, numbers as their stored value (up to 15 significant digits; display formats such as currency or rounding are not applied), booleans, dates as ISO 8601, formula cached values, merged cells as spans, images, core properties. |
